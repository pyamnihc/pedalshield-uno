...
ISR(TIMER1_CAPT_vect)
{
  ...
  if (!bt_count) {
    ...
    if (!(bt_stat & (1 << UP)) && (!(bt_stat & (1 << DN)))) {
      ...
      up_flag ? (up_thresh >= 32767 ? up_thresh = 32767 : up_thresh += 32) : 
                (dn_thresh >= 0 ? dn_thresh = 0 : dn_thresh += 32);
    }
    else if (!(bt_stat & (1 << DN))) {
      up_flag ? (up_thresh <= 0 ? up_thresh = 0 : up_thresh -= 32) : 
                (dn_thresh <= -32768 ? dn_thresh = -32768 : dn_thresh -= 32);
    }
    bt_count = BT_MAX;
  }

  input = input > 0 ? (input > up_thresh ? up_thresh : input) : 
                      (input < dn_thresh ? dn_thresh : input);
